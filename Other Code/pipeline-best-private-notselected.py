...
def preprocessor(dataset):
    ...
    train["month"] = train["date_first_booking"].apply(lambda x: int(x[5:7]))
    test["month"] = test["date_first_booking"].apply(lambda x: int(x[5:7]))
    ...
    test['first_affiliate_tracked'] = test['first_affiliate_tracked'].fillna('none')
    ...
    X["month"] = train["month"].apply(lambda x: (x/12) - 0.5 )
    X_test['month'] = test["month"].apply(lambda x: (x/12) - 0.5 )
    
    age_mean = train["age"].mean()
    age_max = 18
    age_min = 80
    X['age'] = train['age'].apply(lambda x: (x - age_mean)/(age_max - age_min))
    X_test['age'] = test['age'].apply(lambda x: (x - age_mean)/(age_max - age_min))
...
model = xgb.XGBClassifier( n_estimators = 50 , eta = 0.1, reg_lambda = 190 , max_depth = 3, tree_method = "gpu_hist")
